...
### 3. Install openwakeword (special handling required)

> ⚠️ **Important:** `openwakeword` depends on `tflite-runtime`, which **does not support Python 3.12+** on most platforms. Install it without dependencies, then use `onnxruntime` as the inference backend instead.
...
**Solution:** Install without dependencies and use ONNX:
```bash
pip install openwakeword --no-deps
pip install onnxruntime
```
